...
module.exports = function (dependency, version, callback) {
  ...
  domain.run(function () {
    ...
    function mkdirDependency (then) {
      $('fs').mkdir($('path').join(process.env.PWD, 'dude-js', 'dependencies', dependency),
        function (error) {
    ...
    function install () {
      ...
      var dest = $('path').join(process.env.PWD, 'dude-js', 'tmp', $('path').basename(url));

      $('./download')(url, dest, domain.intercept(function () {
        $('./extract')(dest, $('path').join(process.env.PWD, 'dude-js', 'dependencies', 'mongodb'),
          domain.intercept(function () {
