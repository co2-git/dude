...
module.exports = function (service, options, callback) {
  ...
  domain.run(function () {
    ...
    // If options arguments is an array, pass it directly
    if ( $('util').isArray (options) ) {
      args = options;
    }

    // Otherwise transform options to fit arguments syntax
    else {
      for ( var arg in Service.start[startCandidate].arguments ) {
        /* Check forr required arguments -- exit if is required and is missing */
        if ( Service.start[startCandidate].arguments[arg].required && ! options[arg] ) {
          throw new Error('Missing required argument: ' + arg);
        }

        /* Populate arguments to be passed to start script */
        if ( options[arg] ) {
          switch ( Service.start[startCandidate]['arguments-transform'] ) {
    ...
    spawn.on('exit', function (signal) {
      ...
      var error = new Error('Got service error! Service ' + service + ' got signal/code ' + signal);
