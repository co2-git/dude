...
/**
 *  @runner can be either a string (in which case it is either a log ID or a service/script name) or boolean true (in which case it means stop all running services/scripts)
 *
 *  @options
 *  {
...
module.exports = function (
  ...
  domain.run(function () {
    ...
    function stop (runner, cb) {
      // If runner is a zombie, that means its PID has been killed but somehow not its service file
      if ( runner.zombie ) {
        // In which case, we force remove the service file
        $('fs').unlink($('path').join(process.cwd(), 'dude-js', 'services', runner.log + '.json'),
          domain.intercept(function () {
            cb(null, runner);
          }));
      }

      // If not a zombie, kill the bastard
      else {
